...
const AdminDashboard = () => {
  ...
  const [feedbackCount, setFeedbackCount] = useState(0); // New state
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Note: Update your backend dashboard stats view to return total_feedback
        ...
        // Fetch Feedback count specifically if not in profileData
        const feedbackRes = await fetch('http://localhost:8000/api/operations/admin-feedback/', {
            headers: { Authorization: `Bearer ${token}` }
        });
    ...
    <div className="flex min-h-screen bg-[#f4f6f9]">
      ...
      <aside className="w-64 bg-[#343a40] fixed h-full text-[#c2c7d0] shadow-lg overflow-y-auto">
        ...
        <nav className="py-2">
          {[
            ...
            /* UPDATED FEEDBACK LINKS TO POINT TO ONE HUB */
            { name: 'Staff Feedback', path: '/admin-feedback', icon: 'fas fa-comments' },
            { name: 'Student Feedback', path: '/admin-feedback', icon: 'fas fa-comment-dots' },
          ].map((item, i) => (
            <button
              key={i}
      ...
      <main className="ml-64 flex-1">
        <header className="bg-white p-4 shadow-sm border-b sticky top-0 z-10">
          <h2 className="text-gray-700 text-lg uppercase font-bold tracking-tight">Admin Dashboard</h2>
        </header>

        <div className="p-6">
          {/* UPDATED STAT BOXES (5 Columns to accommodate Feedback) */}
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-8">
            ...
            />
            {/* NEW FEEDBACK STAT BOX */}
            <StatBox
              title="Total Feedback"
              count={feedbackCount}
              color="bg-[#605ca8]" // Purple-ish color
              onViewMore={() => navigate('/admin-feedback')}
            />
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
             {/* Charts remain unchanged */}
             <ChartCard title="Staff vs Students" headerColor="bg-[#dd4b39]">
            ...
            <ChartCard title="Subjects per Course" headerColor="bg-[#00a65a]">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
...
const StatBox = ({ title, count, color, onViewMore }) => (
  <div className={`${color} text-white rounded shadow-lg flex flex-col overflow-hidden transition-transform hover:scale-[1.02]`}>
    <div className="p-6 flex flex-col items-center justify-center text-center">
      <span className="text-[10px] font-bold uppercase tracking-widest opacity-90 mb-2">
        {title}
      </span>
    ...
    {onViewMore && (
      <button
        onClick={onViewMore}
        className="pb-3 text-[10px] font-semibold flex items-center justify-center gap-1.5 bg-black/10 hover:bg-black/20 py-2"
      >
...
const ChartCard = ({ title, headerColor, children }) => (
  <div className="bg-white rounded shadow border border-gray-200">
    <div className={`${headerColor} px-4 py-2 text-white text-[10px] uppercase font-bold`}>
      {title}
    </div>
